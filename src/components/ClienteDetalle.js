...
function ClienteDetalle() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 sm:px-6 lg:px-8">
      ...
      <p className="text-sm text-gray-600 mb-4 sm:text-base">Tipo de Cliente: {cliente.tipoCliente}</p>

      {/* Mostrar entrenadores asignados al cliente */}
      {cliente.entrenadores && cliente.entrenadores.length > 0 ? (
        <p className="text-sm text-gray-600">Entrenador/Fisio: {cliente.entrenadores.join(" / ")}</p>
      ) : (
      ...
      {bonosActivos.length > 0 ? (
        bonosActivos.map((bono) => (
          <div key={bono.id} className="border rounded-lg p-4 mb-4 bg-white shadow-md w-full sm:w-auto">
            ...
            <div className="mt-4 space-y-2 sm:space-y-0 sm:space-x-2 flex flex-col sm:flex-row">
              <button className="bg-indigo-500 text-white px-3 py-2 rounded-md w-full sm:w-auto" onClick={() => registrarAsistencia(bono.id, bono.sesionesRestantes)}>
                Registrar Asistencia
              </button>
              <button className="bg-green-500 text-white px-3 py-2 rounded-md w-full sm:w-auto" onClick={() => actualizarEstadoPago(bono.id, "Pagado")}>
                Marcar como Pagado
              </button>
              <button className="bg-red-500 text-white px-3 py-2 rounded-md w-full sm:w-auto" onClick={() => actualizarEstadoPago(bono.id, "Pendiente")}>
        ...
        <div>
          ...
          <Link to="/bonos" className="text-indigo-600 hover:underline">Añadir Bono Nuevo</Link>
        </div>
      )}
      ...
      {bonosGastados.length > 0 && (
        <>
          ...
          {bonosGastados.map((bono) => {
            ...
            const titulo = `Bono gastado${ultimaFecha ? ` - Última sesión: ${ultimaFecha.toLocaleDateString()}` : ""} - Estado de Pago: ${estadoPago}`;
            ...
            return (
              <div key={bono.id} className={`border rounded-lg p-4 mb-4 shadow-md ${fondoBono}`}>
                <div onClick={() => toggleBonoDesplegado(bono.id)} className="cursor-pointer flex items-center justify-between text-black-400 hover:text-indigo-800">
                  <span>{titulo}</span>
                  {bonosDesplegados[bono.id] ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                </div>
                ...
                {bonosDesplegados[bono.id] && (
                  <div className="mt-2">
                    ...
                    <label className="block mt-2">
                      Estado de Pago:
                      <select value={bono.estadoPago} onChange={(e) => actualizarEstadoPago(bono.id, e.target.value)} className="ml-2 bg-white border border-gray-300 rounded-md">
